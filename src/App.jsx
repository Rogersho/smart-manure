...
export default function App() {
  ...
  const handleStart = () => {
    if (!selectedLand || !selectedManure || !selectedCrop) return;
    setIsProcessing(true);
    setResult(null);
    
    // Simulate machine processing time
    setTimeout(() => {
      const mix = determineOptimalMix(selectedLand, selectedManure, selectedCrop);
      setResult(mix);
        ...
        <header className="flex flex-col md:flex-row items-center justify-between mb-8 pb-6 border-b-4 border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-emerald-400 font-bold tracking-widest text-sm uppercase">Smart-Mix Bio-Optimizer</h2>
            </div>
          </div>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <section className="bg-slate-800 border-4 border-slate-700 p-6 relative">
              ...
              <h3 className="text-xl font-bold mb-4 uppercase tracking-wide opacity-90 mt-2 text-slate-300">Select Waste Source</h3>
              <div className="grid grid-cols-3 gap-4">
                {Object.values(MANURE_DATABASE).map(man => (
                  <button
                    key={man.id}
                    onClick={() => setSelectedManure(man.id)}
                    className={`flex flex-col items-center justify-center p-4 border-4 transition-all duration-200 ${
                      selectedManure === man.id 
                        ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' 
                        : 'border-slate-700 bg-slate-900 text-slate-400 hover:border-slate-500 hover:text-white'
                    }`}
                  >
            ...
            <section className="bg-slate-800 border-4 border-slate-700 p-6 flex items-center justify-center gap-8 shadow-inner">
               <button 
                onClick={handleStart}
                disabled={!selectedLand || !selectedManure || !selectedCrop || isProcessing}
                className={`relative overflow-hidden w-32 h-32 rounded-full font-bold uppercase tracking-widest text-lg flex flex-col items-center justify-center border-8 transition-all active:scale-95 ${
                  !selectedLand || !selectedManure || !selectedCrop
